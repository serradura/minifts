---
type: Decision
title: MiniFTS's Role in okf
description: MiniFTS exists to give okf a fast, dependency-free full-text search backend so requiring SQLite + FTS5 can be postponed as long as possible.
tags: [okf]
...
MiniFTS was built to answer a specific need in okf: give the base gem a capable
...
step, a C toolchain, a platform matrix. MiniFTS trades peak scale for zero
...
ceiling, FTS5 remains the intended escape hatch; MiniFTS's job is to *move the
...
The [okf-vs-minifts benchmark](/benchmarks/okf-vs-minifts.md) measures this
directly: against okf's *current* search — a linear substring scan with no index —
minifts sustains **~44–56× the query throughput** over the real `@okf` bundle,
...
[3] [okf-vs-minifts](/benchmarks/okf-vs-minifts.md) — the benchmark-ips comparison and its full results.
